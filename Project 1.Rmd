...
The bootstrap t interval is calculated by first running a parametric bootstrap simulation of a Binomial random sample $B$ times and calculating $\hat{p}_{boot}$ for each of those $B$ bootstrap samples. Then, we will run a secondary bootstrap $B$ times where we will find $\hat{p}_{boot}^*$, which will allow us to find our standard error of $\hat{p}_{boot}$, which we can then use to find our $t$-statistic in order to calculate our confidence interval.
...
```{r}
### Display first 10 observations of each data frame
head(wald_df, n = 10)
head(adjwald_df, n = 10)
head(clopper_df, n = 10)
head(score_df, n = 10)
head(rawboot_df, n = 10)
head(tboot_df, n = 10)
```
...
In this section, we will create plots of coverage probabilities as well as average interval width for each different confidence interval method.
...
In this section, we will separate each data frame containing our final results into three separate data frames, one for each different value of $n$. We will then calculate the mean proportion of intervals that capture the true $p$ as well as the mean average width of the intervals for each value of $n$ for each confidence interval method.

## Wald Interval

See below for results from the Wald interval.
...
See below for results from the Adjusted Wald interval.
...
See below for results from the Clopper-Pearson interval.
...
See below for results from the Score interval.
...
See below for results from the bootstrap raw percentile interval.
...
See below for results from the bootstrap t interval.
