...
###############################################################################
###############################################################################
# I have the function above "rawboot_CI" that works for calculating the 
# raw percentile interval. Below I am trying to get a function working
# that combines both the raw interval and t interval into a single 
# function which is what the instructions say to do. So far, I haven't
# quite been able to get it to work.
###############################################################################
###############################################################################

boot_CI <- function(y, n, alpha = 0.05, B = 100){
  # Finding p_hat, i.e. the sample proportion
  p_hat <- y/n
  ...
  # Here, we will use a parametric bootstrap!
    ...
    # Secondary bootstrap to find SEs needed for t-statistics
    secondary_boot <- replicate(B, {
      data2 <- rbinom(1, size = n, prob = p_hat)
    ...
    # Having an SE very close to 0 will cause problems for us
    # In these cases, we can set those interval values to be 
  ...
  # Return the two different intervals!
  return(list(
    rawboot_int = c(raw_lower, raw_upper),
    tboot_int = c(boot_lower, boot_upper)
  ))
}

###############################################################################
###############################################################################
# THIS CODE BELOW IS ONLY FOR TESTING THE FUNCTION ABOVE BEFORE ADDING THE 
# BOOTSTRAP T INTERVAL TO THE MONTE CARLO SECTION / NESTED LOOP !!!!!!
# THIS IS NOT PART OF THE FINAL SUBMISSION.
# CODE FOR ALL OTHER FIVE CI METHODS HAS BEEN COMPLETED AND IS
# INCLUDED IN THE MONTE CARLO SIMULATION CODE. :)
###############################################################################
###############################################################################
...
tboot_df <- matrix(ncol = 6,nrow = 0, dimnames = list(NULL, c("n", "p", "avg_wdith",
...
for (n in n_values){
  for (p in p_values){
    ...
    for (i in 1:N){
      y <- data[i] 
      CI <- boot_CI(y = y, n = n, alpha = 0.05, B = 10)
      lower <- CI[[2]][1]
      upper <- CI[[2]][2]
      ...
      tboot_allCIs <- rbind(tboot_allCIs, c(n, p, lower, upper))
    }
    ...
    tboot_df <- rbind(tboot_df, c(n, p, avg_width, prop_captured, 
